...
function Get-MerakiMXPorts {
    ...
    .EXAMPLE
        Get-MerakiMXPorts -apiKey $apiKey -networkID "123456"
...
[CmdletBinding()]
    ...
    if($null -ne $devices){
    $response = Get-MerakiAPI -apiKey $apiKey -path "networks/$($networkID)/appliance/ports"
    return $response
    }
}